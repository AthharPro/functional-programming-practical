...
object prac4_3{
   ...
   def main(args: Array[String]): Unit = {
      ...
      println(formatNames(name2)(name => toUpper(name.substring(0,2)) + name.substring(2)))  
      println(formatNames(name3)(toLower))
      println(formatNames(name4)(name => name.substring(0,name.length - 1) + toUpper(name.substring(name.length - 1)))) 
   }
}